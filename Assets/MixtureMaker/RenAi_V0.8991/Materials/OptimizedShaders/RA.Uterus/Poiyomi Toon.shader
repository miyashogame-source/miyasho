Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/0830106d5be62724a960cd3c71744ee0"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				lightMap.y *= lerp(1.0, shadowAttenuationNoStrength, 0.503);
				...
				if (1.0)
				{
					lns.x = poiEdgeNonLinearNoSaturate(lightMap.x, 0.5, 0.204 * blurMap.r);
					lns.y = poiEdgeNonLinearNoSaturate(lightMap.y, 0.0, 0.834 * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(lightMap.z, 0.25, 0.1 * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, 0.5, 0.204 * blurMap.r, 0.0);
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 0.204 * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(lightMap.y, 0.0, 0.834 * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(lightMap.z, 0.25, 0.1 * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 0.204 * blurMap.r, 0.0);
				}
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, 0.896);
				#ifndef POI_PASS_ADD
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.3371702,0.04666467,0.04666467,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.3371702,0.04666467,0.04666467,1).a;
				if (2.0)
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
