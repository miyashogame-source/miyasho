Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/b3f8ef654136bcd4490fed0c0a57edc5"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				lightMap.x *= lerp(1.0, shadowAttenuationNoStrength, 0.669);
				...
				if (1.0)
				{
					lns.x = poiEdgeNonLinearNoSaturate(lightMap.x, 0.824, 0.904 * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, 0.824, 0.904 * blurMap.r, 0.0);
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, 0.824, 0.904 * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(lightMap.y, 0.5, 0.3 * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(lightMap.z, 0.25, 0.1 * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, 0.824, 0.904 * blurMap.r, 0.0);
				}
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (float4(0.6202959,0.6525444,0.7199211,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.6202959,0.6525444,0.7199211,1).rgb;
				}
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
				getMatcapUV(matcapUV, float4(0,0,0,0).xy, 1.0, 1.0, float4(0.458,0,0,0).xy, 0.0, 0.54, normal0, poiCam, poiLight, poiMesh, 1.0, matcapALD);
				if (1.0)
				{
					float mipCount0 = 9;
					...
					matcap = UNITY_SAMPLE_TEX2D_SAMPLER_LOD(_Matcap, _trilinear_repeat, TRANSFORM_TEX(matcapUV, _Matcap), matcapSmoothness) * float4(poiThemeColor(poiMods, float4(0.2702231,0.2702231,0.2702231,1).rgb, 0.0), float4(0.2702231,0.2702231,0.2702231,1).a);
				}
				else
				{
					matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(poiThemeColor(poiMods, float4(0.2702231,0.2702231,0.2702231,1).rgb, 0.0), float4(0.2702231,0.2702231,0.2702231,1).a);
				}
				#else
				matcap = float4(poiThemeColor(poiMods, float4(0.2702231,0.2702231,0.2702231,1).rgb, 0.0), float4(0.2702231,0.2702231,0.2702231,1).a);
				#endif
				matcapIntensity = 1.0;
				#ifdef POI_AUDIOLINK
				if (matcapALD.matcapALEnabled > 0)
				...
				blendMatcap(poiLight, poiFragData, poiMods, 1.0, 0.0, 0.0, 0.0, 0.559, 0.0, matcap, matcapMask, 0.0, 1.0, 0.0, 2.0, matcapALD);
				#endif
				#ifdef POI_MATCAP2
				matcapALD.matcapALEnabled = 0.0;
				matcapALD.matcapALAlphaAddBand = 0.0;
				...
				blendMatcap(poiLight, poiFragData, poiMods, 0.097, 0.0, 0.285, 0.041, 0.0, 0.0, matcap3, matcap3Mask, 0.0, 0.0, 0.0, 2.0, matcapALD);
				#endif
			}
			#endif
			#ifdef POI_SUBSURFACESCATTERING
			void applySubsurfaceScattering(in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiFragData poiFragData)
			{
				...
				float flTDot = pow(saturate(dot(poiCam.viewDir, -vLTLight)), 2.5) * 1.0;
				#ifdef UNITY_PASS_FORWARDBASE
				float3 fLT = (flTDot) * saturate(SSS.a + - 1 * -0.28);
				#else
				float3 fLT = poiLight.additiveShadow * (flTDot) * saturate(SSS.a + - 1 * -0.28);
				#endif
				#if defined(POINT) || defined(SPOT)
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(1,0.003346533,0.9535654,1) * SSS.rgb * lerp(1, poiFragData.baseColor, 0.639);
				#endif
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(1,0.003346533,0.9535654,1) * SSS.rgb * poiLight.attenuation * lerp(1, poiFragData.baseColor, 0.639);
			}
			#endif
			#ifdef POI_ENVIRORIM
			void applyEnvironmentRim(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				float enviroRimAlpha = saturate(1 - smoothstep(min(0.162, 0.373), 0.373, poiCam.vDotN));
				0.0 *= 1.7 - 0.7 * 0.0;
				...
				{
					float4 reflectionData0 = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], 0.0 * UNITY_SPECCUBE_LOD_STEPS);
					float3 reflectionColor0 = DecodeHDR(reflectionData0, unity_SpecCube0_HDR);
					float4 reflectionData1 = UNITY_SAMPLE_TEXCUBE_SAMPLER_LOD(unity_SpecCube1, unity_SpecCube0, poiMesh.normals[1], 0.0 * UNITY_SPECCUBE_LOD_STEPS);
				...
				{
					float4 reflectionData = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, poiMesh.normals[1], 0.0 * UNITY_SPECCUBE_LOD_STEPS);
				...
				#endif
				float3 envRimCol = lerp(0, max(0, (enviroRimColor - 0.0) * poiFragData.baseColor), enviroRimAlpha).rgb * enviroMask * 0.219;
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.9471843,0.9471843,1,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.9471843,0.9471843,1,1).a;
				if (2.0)
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
