Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/41c231fa6e52d15438cc34ea80079bb3"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float roughness = 1 - (clothmapsample.a * 0.75);
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				if (0.025 || (2.0 == 2 && 0.1 != 0))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(1,0.4254881,0.9209429,1).rgb, 0.0), float4(1,0.4254881,0.9209429,1).a);
					#ifndef POI_SHADOW
			...
			void applySubsurfaceScattering(in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiFragData poiFragData)
			{
				...
				SSS.a = 1 - SSS.a;
				#endif
				float3 vLTLight = poiLight.direction + poiMesh.normals[0] * 0.318;
				float flTDot = pow(saturate(dot(poiCam.viewDir, -vLTLight)), 2.3) * 1.0;
				#ifdef UNITY_PASS_FORWARDBASE
				float3 fLT = (flTDot) * saturate(SSS.a + - 1 * -0.14);
				#else
				float3 fLT = poiLight.additiveShadow * (flTDot) * saturate(SSS.a + - 1 * -0.14);
				#endif
				#if defined(POINT) || defined(SPOT)
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(1,0,0.3855847,1) * SSS.rgb * lerp(1, poiFragData.baseColor, 0.0);
				#endif
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(1,0,0.3855847,1) * SSS.rgb * poiLight.attenuation * lerp(1, poiFragData.baseColor, 0.0);
			}
			#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.03688945,0.03688945,0.03688945,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.03688945,0.03688945,0.03688945,1).a;
				if (2.0)
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
