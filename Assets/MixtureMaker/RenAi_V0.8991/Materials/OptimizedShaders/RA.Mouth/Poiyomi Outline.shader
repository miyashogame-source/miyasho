Shader "Hidden/Locked/.poiyomi/Old Versions/8.0/Poiyomi Outline/41e5077ed6044824db64b4ca98ada760"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			v2f vert(appdata v)
			{
				...
				if (0.0)
				{
					distanceOffset *= min(distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, o.localPos).xyz), 9999.0);
				}
				...
				if (1.0)
				{
					if (LumaIsAvailable())
					{
						lineWidth += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, LumaData(0.578, 0.515)[0.0 / 2]);
					}
			...
			float2 calculatePolarCoordinate(in PoiMesh poiMesh)
			{
				...
				float angle = atan2(delta.x, delta.y) * 1.0 / 6.28 * 1.0;
			...
			{
				float3 x0, x1, x2;
				float3 conv = lerp(GreenWrapSH(wrap), GreenWrapSHOpt(wrap), 0.0); // Should try optimizing this...
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (1.0 == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * 0.5 * poiLight.vAttenuation[index], smoothstep(0.0, 0.5, .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0)), 1.0);
				...
				poiLight.occlusion = lerp(1, AOMaps.r, 1.0) * lerp(1, AOMaps.g, 0.0) * lerp(1, AOMaps.b, 0.0) * lerp(1, AOMaps.a, 0.0);
				#else
				...
				if (0.0 == 3)
				{
					...
					poiLight.indirectColor = max(poiLight.indirectColor, 0.0);
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / 0.0)));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / 0.0)));
				}
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.8760344,0.6439822,0.6439822,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.8760344,0.6439822,0.6439822,1).a;
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				clip(1.0 - 0.01);
				float OutlineMask = tex2D(_OutlineMask, TRANSFORM_TEX(poiMesh.uv[0.0], _OutlineMask) + _Time.x * float4(0,0,0,0)).r;
				if (0.0)
				{
					float lineWidth = 0.05;
					#ifdef POI_AUDIOLINK
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.1778885,0.05126945,0.05780542,1).rgb, 0.0), float4(0.1778885,0.05126945,0.05780542,1).a);
				poiFragData.baseColor = lerp(col, col * poiFragData.baseColor, 0.0);
				if (1.0 == 2)
				{
				...
				if (0.0)
				{
					poiFragData.baseColor = hueShift(poiFragData.baseColor, 0.0 +0.0 * _Time.x);
				}
				float emission = 0.0;
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
