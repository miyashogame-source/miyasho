Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/bac31bb48e8e8cc498e76439f21075d6"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.1261767,0.1418043,0.1807605,0).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.1261767,0.1418043,0.1807605,0).a;
				if (2.0)
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
				...
				#endif
				if (6.0 == POI_MODE_OPAQUE)
				{
				}
				clip(poiFragData.alpha - 0.0);
				if (6.0 == POI_MODE_CUTOUT && !0.0)
				{
					poiFragData.alpha = 1;
				}
