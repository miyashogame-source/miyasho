Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/350d743f77aa9534abc4ba2f0c5958ee"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				lightMap.x *= lerp(1.0, shadowAttenuationNoStrength, 0.676);
				...
				if (1.0)
				{
					lns.x = poiEdgeNonLinearNoSaturate(lightMap.x, 0.5, 0.719 * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, 0.5, 0.719 * blurMap.r, 0.0);
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 0.719 * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(lightMap.y, 0.5, 0.3 * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(lightMap.z, 0.25, 0.1 * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 0.719 * blurMap.r, 0.0);
				}
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (float4(0.6172066,0.6514057,0.7230552,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.6172066,0.6514057,0.7230552,1).rgb;
				}
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, poiMods, 0.097, 0.0, 0.285, 0.041, 0.0, 0.0, matcap, matcapMask, 0.0, 0.0, 0.0, 2.0, matcapALD);
				#endif
			}
			#endif
			#ifdef POI_SUBSURFACESCATTERING
			void applySubsurfaceScattering(in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiFragData poiFragData)
			{
				...
				#endif
				#if defined(POINT) || defined(SPOT)
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(1,0.003346531,0.9535654,1) * SSS.rgb * lerp(1, poiFragData.baseColor, 0.639);
				#endif
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(1,0.003346531,0.9535654,1) * SSS.rgb * poiLight.attenuation * lerp(1, poiFragData.baseColor, 0.639);
			}
			#endif
			#ifdef POI_ENVIRORIM
			void applyEnvironmentRim(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				float enviroRimAlpha = saturate(1 - smoothstep(min(0.135, 0.339), 0.339, poiCam.vDotN));
				0.0 *= 1.7 - 0.7 * 0.0;
				...
				#endif
				float3 envRimCol = lerp(0, max(0, (enviroRimColor - 0.0) * poiFragData.baseColor), enviroRimAlpha).rgb * enviroMask * 0.541;
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.9473069,0.9473069,1,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.9473069,0.9473069,1,1).a;
				if (2.0)
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
