Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/0f800fd8f30d10b41b18870eb55759d1"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				lightMap.x *= lerp(1.0, shadowAttenuationNoStrength, 0.344);
				lightMap.y *= lerp(1.0, shadowAttenuationNoStrength, 0.146);
				...
				if (1.0)
				{
					lns.x = poiEdgeNonLinearNoSaturate(lightMap.x, 0.407, 0.156 * blurMap.r);
					lns.y = poiEdgeNonLinearNoSaturate(lightMap.y, 0.0, 0.924 * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(lightMap.z, 0.25, 0.1 * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, 0.407, 0.156 * blurMap.r, 0.0);
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, 0.407, 0.156 * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(lightMap.y, 0.0, 0.924 * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(lightMap.z, 0.25, 0.1 * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, 0.407, 0.156 * blurMap.r, 0.0);
				}
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (float4(0.9684471,0.7138588,0.6973871,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.9684471,0.7138588,0.6973871,1).rgb;
				}
				if (float4(0.2961383,0.3189064,0.7835379,0.2705882).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.2961383,0.3189064,0.7835379,0.2705882).rgb;
					lns.y = float4(0.2961383,0.3189064,0.7835379,0.2705882).a - lns.y * float4(0.2961383,0.3189064,0.7835379,0.2705882).a;
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!0.0)
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.8962696,0.1778885,0.9046614,1).rgb, 0.0);
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * 5.0).rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.8962696,0.1778885,0.9046614,1).rgb, 0.0);
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(0.8962696,0.1778885,0.9046614,1).rgb, 0.0);
				#endif
				if (0.0)
				{
			...
			void applySubsurfaceScattering(in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiFragData poiFragData)
			{
				...
				float flTDot = pow(saturate(dot(poiCam.viewDir, -vLTLight)), 1.0) * 0.228;
				...
				#endif
				#if defined(POINT) || defined(SPOT)
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(1,0.03050088,0,1) * SSS.rgb * lerp(1, poiFragData.baseColor, 0.0);
				#endif
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(1,0.03050088,0,1) * SSS.rgb * poiLight.attenuation * lerp(1, poiFragData.baseColor, 0.0);
			}
			#endif
			#ifdef POI_BACKLIGHT
			...
			{
				float3 backlightColor = float4(0.1298693,0.141649,0.2054128,1).rgb;
				#if defined(PROP_BACKLIGHTCOLORTEX) || !defined(OPTIMIZER_ENABLED)
				...
				float backlightLN = dot(normalize(-headDir * 0.78 + poiLight.direction), normal) * 0.5 + 0.5;
				#if defined(POINT) || defined(SPOT)
				if (1) backlightLN *= saturate(poiLight.additiveShadow);
				#else
				if (1) backlightLN *= saturate(poiLight.attenuation);
				#endif
				backlightLN = poiEdgeLinear(backlightLN, 0.335, 0.03);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(1,0.9495302,0.9061305,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(1,0.9495302,0.9061305,1).a;
				if (2.0)
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				...
				poiFragData.baseColor = lerp(col, col * poiFragData.baseColor * lerp(1, poiFragData.baseColor, 0.0), 0.443);
				float4 outlineColor = float4(0.3324516,0.09530746,0.09758735,1);
				#ifdef POI_AUDIOLINK
